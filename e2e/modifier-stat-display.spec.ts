...
test.describe('Modifier stat display and expiry', () => {
  ...
  test('should show armor bonus when soldierModifier is active', async ({ page }) => {
    ...
    // and the "+2" text should be visible inside the armor badge
    const armorBadge = page.getByTestId('stat-badge-armor').first();
    await expect(armorBadge).toContainText('+2', { timeout: 5000 });
  });
  ...
  test('should NOT show bonus when no modifiers active', async ({ page }) => {
    ...
    // The armor badge should NOT contain a "+2" bonus
    const armorBadge = page.getByTestId('stat-badge-armor').first();
    await expect(armorBadge).not.toContainText('+2', { timeout: 3000 });
  });
  ...
  test('should show negative bonus (red) when debuff is applied', async ({ page }) => {
    ...
    // Speed stat should show "x0.5" modifier inside the speed badge
    const speedBadge = page.getByTestId('stat-badge-speed').first();
    await expect(speedBadge).toContainText('x0.5', { timeout: 5000 });
  });
  ...
  test('should remove expired modifier after advancing turns', async ({ page }) => {
    ...
    // So "+1" should be visible inside the range badge
    const rangeBadge = page.getByTestId('stat-badge-range').first();
    await expect(rangeBadge).toContainText('+1', { timeout: 5000 });
  ...
  test('should show debuff effect on stats when activeDebuff is present', async ({ page }) => {
    ...
    // The armor badge should show -1 bonus (red)
    const armorBadge = page.getByTestId('stat-badge-armor').first();
    await expect(armorBadge).toContainText('-1', { timeout: 5000 });
  });
  ...
  test('should NOT show expired debuff in stats', async ({ page }) => {
    ...
    // The speed badge should NOT show x0.5 since the debuff is expired
    // (isModifierActive(1, 1, 5) = 5 <= 2 → false)
    const speedBadge = page.getByTestId('stat-badge-speed').first();
    await expect(speedBadge).not.toContainText('x0.5', { timeout: 3000 });
  });
});
