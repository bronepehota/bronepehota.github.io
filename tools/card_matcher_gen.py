...
.wrap{position:relative;z-index:2;max-width:1320px;margin:0 auto;padding:18px}
...
.main{display:grid;grid-template-columns:minmax(340px,44%) 1fr;gap:16px;align-items:start}
...
.cardpane img{display:block;width:var(--cw,460px);transition:width .12s}
...
/* soldier rows */
.row-grid{display:grid;grid-template-columns:34px 116px repeat(7,minmax(44px,1fr)) 90px;gap:7px 9px;align-items:center}
.thead{padding:0 10px 5px;border-bottom:1px solid var(--border);margin-bottom:3px}
.thead .h{font-family:'Oswald';font-size:10px;letter-spacing:1px;color:var(--muted);text-transform:uppercase;text-align:center}
...
.s-num{font-family:'Black Ops One';font-size:16px;color:var(--ru2);text-align:center;height:30px;display:grid;place-items:center;background:#0b0d10;border:1px solid var(--border2);border-radius:3px}
.s-photo{position:relative}
...
.cell input,.cell select{background:#0b0d10;border:1px solid var(--border2);color:var(--bone);font-family:'JetBrains Mono';font-weight:500;font-size:13px;padding:5px 3px;border-radius:3px;width:100%;text-align:center}
.cell input:focus,.cell select:focus{outline:none;border-color:var(--ru)}
.cell.photo-sel select{text-align:left;color:var(--ru2)}
...
function render(){
  ...
  const card=d.card?`<div class="cv-controls">
      <label>зум <input type="range" min="0.6" max="2.6" step="0.1" value="${zoom}" oninput="zoom=+this.value;document.getElementById('cardimg').style.setProperty('--cw',(460*zoom)+'px');persist()"></label><b>${zoom.toFixed(1)}×</b>
    ...
    <div class="cardpane" id="cardpane"><img id="cardimg" src="${d.card}" style="--cw:${460*zoom}px" onload="focusSoldier(activeN)"><div class="cardcursor" id="cardcursor" style="display:none"></div></div>
  ...
  const rows=d.soldiers.map((s,i)=>{
    ...
    const imgSel=Array.from({length:d.soldiers.length},(_,k)=>k+1).map(o=>`<option value="${o}" ${n===o?'selected':''}>${o}</option>`).join('');
    return `<div class="row-grid soldier ${conflict?'conflict':''}" onmouseenter="activeN=${s.num};focusSoldier(${s.num});document.querySelectorAll('.soldier').forEach(e=>e.classList.remove('active'));this.classList.add('active')">
      <div class="s-num">${s.num}</div>
      <div class="s-photo"><img src="${src}" onclick="zoomImg('${src}','боец ${s.num} · фото ${n}')">${conflict?`<span class="dup">dup ${n}</span>`:''}</div>
      <div class="cell"><input value="${s.rank??''}" onchange="setSoldier(${i},'rank',this.value)"></div>
      <div class="cell"><input value="${s.speed??''}" onchange="setSoldier(${i},'speed',this.value)"></div>
      <div class="cell"><input value="${s.range??''}" onchange="setSoldier(${i},'range',this.value)"></div>
      <div class="cell"><input value="${s.power??''}" onchange="setSoldier(${i},'power',this.value)"></div>
      <div class="cell"><input value="${s.melee??''}" onchange="setSoldier(${i},'melee',this.value)"></div>
      <div class="cell"><select onchange="setSoldier(${i},'modifier',this.value)">${modOpts}</select></div>
      <div class="cell"><input value="${s.armor??''}" onchange="setSoldier(${i},'armor',this.value)"></div>
      <div class="cell photo-sel"><select onchange="setImg(${i},this.value)">${imgSel}</select></div>
    </div>`;
  }).join('');
  document.getElementById('view').innerHTML=`
    ...
    <div class="main">
      <div class="cardview"><div class="cv-title">карточка армлиста</div>${card}</div>
      <div>
        <div class="row-grid thead"><div class="h">#</div><div class="h l">📷 фото</div><div class="h">А</div><div class="h">Ск</div><div class="h">Дальн</div><div class="h">Мощн</div><div class="h">ББ</div><div class="h">Св</div><div class="h">Бр</div><div class="h">📷№</div></div>
...
let activeN=1;
function setImg(i,v){cur().soldiers[i].imgIndex=+v;saveCurrent();render();}
