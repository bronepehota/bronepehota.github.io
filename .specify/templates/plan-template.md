...
- [ ] **Russian UI**: All user-facing text, error messages, tooltips in Russian?
- [ ] **File-Based Storage**: Game data in `src/data/` JSON files? API routes use sync file ops?
- [ ] **Client-Side State**: Army state in `localStorage` with key `bronepehota_army`?
- [ ] **Type Safety**: Types in `src/lib/types.ts`? No `any` without justification?
...
- [ ] **Reference Docs**: Game mechanics verified against `docs/original/official_rules.txt` (tehnolog) or `docs/panov/fan_rules.md` (fan)?
