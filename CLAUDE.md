...
- **`pkill -f next` self-matches**: it kills ANY process whose command line contains "next" — including the shell running `rm -rf .next`, so the compound one-liner above dies silently. Use a specific pattern (`pkill -9 -f 'next dev'` / `next-server`) or run `rm -rf .next` in a separate call from pkill.
