...
| Calculator (standalone бой) | `calculator.spec.ts` | 9 |
...
**Боевая песочница (UnitCombatSandbox)** (`src/components/encyclopedia/UnitDetail/UnitCombatSandbox.tsx`): bottom-sheet на странице юнита энциклопедии. **СПРЯТАНА по решению владельца (2026-08-28): «не так хороша, как реальный бой» — вернуть флагом `SHOW_SANDBOX` в UnitDetailPage.tsx (e2e — describe.skip).** Архитектурно: — кнопка «ПРОВЕРИТЬ БОЕМ» в CTA-панели (**только отряды**; машины — v2). Quick-расчёт без создания армии: действия только выстрел и ближний бой (гранату не экспонируем). Prefill статов первого солдата через `soldierToCombatantData` (`src/lib/combatant-data.ts`); чипы солдат — только если их боевые статы различаются, переключение проталкивается полями (хук берёт `initial` лишь при монтировании) с отложенным `newCalculation` (stale-closure guard). Переиспользует `useStandaloneCombatFlow(initial?)` + боевые компоненты (`ParameterInputs`, `CombatResults`, `DiceInputPopup`, `RulesSelector`) через `CombatantData`-адаптер. Аналитика: `sandbox_open` (unit) при открытии. e2e — `e2e/sandbox.spec.ts` (skip, пока песочница спрятана). Плашка «// РЕЖИМ БОЯ» на главной энциклопедии — тонкая строка-телетайп, АКТИВНА (прятали только песочницу).

**Standalone-калькулятор `/calculator`** (`src/components/calculator/CalculatorPage.tsx`) — восстановлен 2026-09-15 по просьбе игроков (был упразднён 2026-08-28 вместе с переносом в песочницу, которую потом спрятали — игроки остались без ручного расчёта). Тот же движок `useStandaloneCombatFlow` (без армии, ручной ввод статов); внутренние боевые компоненты — разделяемые с реальным боем, НЕ перекрашивать. Обвязка (хедер/табы/панель модификаторов) — язык военного досье: military-палитра, Russo + IBM-mono, квадратные рамки, без скруглений; акценты действий (amber/red/emerald) сохранены — «досье-фасад, боевой интерьер». Кнопка модификаторов — в шапке (плавающая «середина края» запрещена правилами HUD владельца). Свод правил — отдельной строкой под табами (в хедере не влезает на мобиле). Входы: терциарная телеграфная строка `// КАЛЬКУЛЯТОР БОЯ · без армии` под CTA-модулями лендинга (`CTAButton.tsx`); тихая строка под баннером «// РЕЖИМ БОЯ» на хабе энциклопедии (`ArchiveHub.tsx`); тонкая кнопка «Калькулятор · без армии» + микро-строка «в бою статы подтянутся сами» в панели «// В БОЙ» досье юнита (`UnitToBattleCta.tsx`). Все — без analytics-события. e2e — `e2e/calculator.spec.ts` (7 + mobile-viewport describe: подписи табов на 375px, модификаторы в шапке).
