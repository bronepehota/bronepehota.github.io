...
export function TabBar({ activeTab, onTabChange, availableCount, armyCount, factionId, currentCost = 0, pointBudget = 250 }: TabBarProps) {
  const factionColors = {
    polaris: 'bg-red-500',
    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-slate-900/95 backdrop-blur-sm border-t border-slate-700/50 shadow-xl">
      <div className="flex items-stretch h-16 md:h-20 max-w-4xl mx-auto md:border-x md:border-slate-700/50 md:rounded-t-xl">
        ...
        >
          ...
          {activeTab === 'browse' && (
            <div className={cn('absolute bottom-0 left-1/2 -translate-x-1/2 w-16 h-0.5', activeColor)} />
          )}
        </button>
