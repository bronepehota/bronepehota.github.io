...
const ACTION_TABS: Array<{ type: CombatActionType; label: string; shortLabel: string; icon: typeof Target }> = [
  { type: 'shot', label: 'ВЫСТРЕЛ', shortLabel: 'ВЫСТРЕЛ', icon: Target },
  { type: 'melee', label: 'БЛИЖНИЙ БОЙ', shortLabel: 'БЛИЖНИЙ', icon: Sword },
  { type: 'grenade', label: 'ГРАНАТА', shortLabel: 'ГРАНАТА', icon: Bomb },
];

// Тип действия — акценты общие с боевыми компонентами (ParameterInputs,
// CombatResults): фасад досье, интерьер боевой. Квадратные рамки, без скруглений.
const getActionColors = (actionType: CombatActionType | null, isSurpriseAttack?: boolean) => {
  ...
  return colorMap[actionType as keyof typeof colorMap] || {
    primary: 'text-military-taupe',
    border: 'border-military-steel/50',
    bg: 'bg-military-charcoal',
    accent: 'border-military-steel',
    button: 'border-military-steel text-military-taupe'
  };
};
...
export function CalculatorPage() {
  ...
  const hasActiveModifiers = modifierSummary.descriptions.length > 0;

  return (
    <div className="min-h-screen bg-military-dark text-military-sand">
      {/* Шапка-дело: назад + действие + модификаторы. Кнопка модификаторов
          прижата к хедеру — не «плавающая середина» правого края. */}
      <header className="border-b border-military-steel/40">
        <div className="max-w-[600px] mx-auto flex items-center gap-3 px-4 py-3">
          <Link
            href="/"
            aria-label="На главную"
            data-testid="calculator-back"
            className="w-11 h-11 shrink-0 flex items-center justify-center border border-military-steel/50 text-military-taupe hover:border-military-amber hover:text-military-amber transition-colors"
          >
          ...
          <div className="flex-1 min-w-0">
            <h1 className={cn(
              "font-russo text-sm sm:text-base uppercase tracking-[0.15em] sm:tracking-[0.25em] truncate",
              currentAction ? actionColors.primary : "text-military-sand"
            )}>
              {currentAction === 'shot' ? 'ВЫСТРЕЛ' :
               currentAction === 'melee' ? 'БЛИЖНИЙ БОЙ' :
               currentAction === 'grenade' ? 'ГРАНАТА' : 'Калькулятор боя'}
            </h1>
            <div className="font-ibm-mono text-[8px] sm:text-[9px] uppercase tracking-[0.3em] text-military-rust">
              {'// БЕЗ АРМИИ'}
            </div>
          </div>
          ...
          <button
            ...
            data-testid="calculator-modifiers-button"
            aria-label="Модификаторы"
            className={cn(
              "relative w-11 h-11 shrink-0 flex items-center justify-center border transition-colors",
              showModifiers
                ? "border-military-amber text-military-amber bg-military-amber/10"
                : "border-military-steel/50 text-military-taupe hover:border-military-amber hover:text-military-amber"
            )}
          >
            <SlidersHorizontal className="w-5 h-5" />
            {hasActiveModifiers && (
              <span className="absolute top-1 right-1 w-1.5 h-1.5 bg-military-amber" />
            )}
          </button>
        </div>
      </header>

      {/* Табы действия — подписи видны и на мобиле (короткие), полные с sm */}
      <nav className="border-b border-military-steel/40" data-testid="calculator-tabs">
        <div className="max-w-[600px] mx-auto flex">
          {ACTION_TABS.map(({ type, label, shortLabel, icon: Icon }) => {
            const tabColors = getActionColors(type);
            return (
              <button
                key={type}
                data-testid={`calculator-tab-${type}`}
                onClick={() => {
                ...
                className={cn(
                  "flex-1 flex items-center justify-center gap-1.5 py-3 border-b-2 transition-all font-ibm-mono text-[10px] sm:text-xs uppercase tracking-widest min-h-[48px]",
                  currentAction === type
                    ? `${tabColors.primary} ${tabColors.border} ${tabColors.bg}`
                    : "text-military-taupe/60 border-transparent hover:text-military-sand hover:border-military-steel"
                )}
              >
      ...
      {/* Свод правил — отдельной строкой под табами (в хедере не влезает на мобиле) */}
      <div className="max-w-[600px] mx-auto px-4 pt-3" data-testid="calculator-rules">
        <RulesSelector value={rulesVersion} onChange={updateRulesVersion} />
      </div>

      {/* Main content */}
      <div className="max-w-[600px] mx-auto p-4 pt-3 space-y-3 relative">
        ...
        {showModifiers && (
          <div className="fixed inset-0 z-40 flex justify-end" onClick={() => setShowModifiers(false)}>
            ...
            <div
              className="w-80 max-w-[85vw] h-full bg-military-charcoal border-l border-military-steel/60 p-4 overflow-y-auto"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-russo text-xs uppercase tracking-[0.2em] text-military-sand">
                  Модификаторы
                </h3>
                <button
                  onClick={() => setShowModifiers(false)}
                  aria-label="Закрыть"
                  className="w-11 h-11 flex items-center justify-center border border-military-steel/50 text-military-taupe hover:border-military-amber hover:text-military-amber transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
            ...
            <div className="flex gap-2 md:gap-3">
              ...
              {(combatState.actionType === 'shot' || combatState.actionType === 'melee') && (
                <button
                  ...
                  className={cn(
                    'relative h-10 w-10 min-h-[40px] min-w-[40px] md:h-12 md:w-12 md:min-h-[48px] md:min-w-[48px] border-2 flex items-center justify-center shrink-0',
                    'touch-manipulation active:scale-95 transition-all duration-200',
                    combatState.parameters.isSurpriseAttack
                      ? 'bg-purple-600/20 border-purple-500 shadow-lg shadow-purple-500/20'
                      : 'bg-military-charcoal border-military-steel/60 hover:border-military-steel'
                  )}
                  aria-label={combatState.parameters.isSurpriseAttack ? 'Внезапная атака включена' : 'Внезапная атака выключена'}
                >
                  <EyeOff
                    className={cn('transition-colors duration-200', combatState.parameters.isSurpriseAttack ? 'text-purple-400' : 'text-military-taupe')}
                    size={16}
                  />
                  {combatState.parameters.isSurpriseAttack && (
                    <span className="absolute top-0.5 right-0.5 w-1.5 h-1.5 bg-purple-400 animate-pulse" />
                  )}
                </button>
              )}
              ...
              {combatState.actionType === 'shot' && combatantData.type === 'squad' && (
                ...
                >
                  <Crosshair
                    className={cn('transition-colors duration-200', combatState.parameters.isAimedShot ? 'text-cyan-400' : 'text-military-taupe')}
                    size={16}
                  />
                  {combatState.parameters.isAimedShot && (
                    <span className="absolute top-0.5 right-0.5 w-1.5 h-1.5 bg-cyan-400 animate-pulse" />
                  )}
                </button>
              )}
              ...
              <button
                ...
                className={cn(
                  "relative flex-1 font-ibm-mono text-xs md:text-sm font-bold uppercase tracking-wider border-2 transition-all min-h-[44px] md:min-h-[48px]",
        ...
        {combatState.phase === 'APPLY' && (
          <div className="text-center py-8 space-y-4">
            <div className="font-ibm-mono text-sm text-military-taupe">Результат принят</div>
            <button
              onClick={newCalculation}
              className="px-6 py-3 font-ibm-mono font-bold uppercase tracking-widest border-2 border-military-amber/70 bg-military-amber/10 text-military-amber hover:bg-military-amber/20 transition-all min-h-[48px] active:scale-95"
            >
              Новый расчёт
            </button>
          </div>
