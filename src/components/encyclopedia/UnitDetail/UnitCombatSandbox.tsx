...
export function UnitCombatSandbox({ unit, soldiers, onClose }: UnitCombatSandboxProps) {
  ...
  const pickSoldier = (i: number) => {
    if (i === soldierIdx) return; // повторный клик по выбранному чипу — ничего не делаем
    setSoldierIdx(i);
  ...
  return (
    <>
      <div
        ...
        onClick={onClose}
      >
        {/* Панель: мобайл — full-width, приклеена к низу, радиус только сверху;
            md+ — «парящая» карточка-диалог по центру (радиус со всех сторон,
            отступ от нижней кромки 24px). mx-auto поверх inset-x-0 центрирует. */}
        <div
          ref={sheetRef}
          data-testid="unit-combat-sandbox"
          className="fixed inset-x-0 bottom-0 rounded-t-2xl bg-slate-900 border-t-2 border-slate-700 shadow-2xl max-h-[90dvh] overflow-y-auto animate-slideUp md:mx-auto md:max-w-xl md:bottom-6 md:rounded-2xl md:border-2"
      ...
      {/* Ввод недостающих кубиков — SIBLING оверлея, а не ребёнок: у попапа нет
          stopPropagation, и внутри оверлея (onClick={onClose}) любой клик по кубику
          всплывал и закрывал всю песочницу. */}
      {dicePopupField && (
        <DiceInputPopup
          title={
