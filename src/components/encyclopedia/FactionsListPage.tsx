    ...
    <div className="min-h-screen bg-military-dark relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <header className="relative py-6 md:py-16 px-4 overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto mt-10 military-divider" />
        </header>

        {/* Faction dossier cards */}
        <main className="px-4 pb-20">
          <div className="max-w-5xl mx-auto space-y-3 md:space-y-4" data-testid="faction-grid">
            {sorted.map((faction, index) => {
              const colors = getFactionColors(faction.id);
              const unitCount = getUnitsForFaction(faction.id).length;
              const parentFaction = faction.parent ? getParent(faction.id, factions) : null;
              const parentName = parentFaction?.name;
              const parentColor = parentFaction ? getFactionColors(parentFaction.id).primary : colors.primary;
              const isSub = !!faction.parent;
              const subs = getSubFactions(faction.id, factions);
              ...
              return (
                ...
                >
                  {/* Branch connector — sub-faction only: an L-tie back to the parent's column */}
                  {isSub && (
                    <span className="hidden md:block absolute pointer-events-none" aria-hidden
                      style={{ left: '-2rem', top: '-1.75rem', bottom: 0, width: '1px', background: `${parentColor}55` }} />
                  )}
                  {isSub && (
                    <span className="hidden md:block absolute pointer-events-none top-1/2 -translate-y-1/2" aria-hidden
                      style={{ left: '-2rem', width: '2rem', height: '1px', background: `${parentColor}55` }} />
                  )}

                  {/* Color side rail + glow (thinner / dimmer for sub-factions) */}
                  <div
                    className={cn('absolute inset-y-0 left-0', isSub ? 'w-1' : 'w-1.5')}
                    style={{ background: `linear-gradient(180deg, ${colors.primary}, transparent)`, opacity: isSub ? 0.7 : 1 }}
                  />
                  <div
                    className="absolute inset-0 pointer-events-none"
                    style={{ background: `radial-gradient(120% 80% at 0% 0%, ${colors.primary}14, transparent 60%)`, opacity: isSub ? 0.18 : 0.3 }}
                  />

                  <div className={cn('relative flex flex-col md:flex-row', isSub ? 'p-3 md:p-4 gap-3 md:gap-4' : 'p-4 md:p-6 gap-4 md:gap-6')}>
                    {/* Emblem */}
                    <div className={cn('flex md:flex-col items-center md:items-start shrink-0', isSub ? 'gap-3 md:w-16' : 'gap-4 md:gap-3 md:w-40')}>
                      <div
                        className={cn('relative flex items-center justify-center rounded-lg overflow-hidden', isSub ? 'w-12 h-12 md:w-14 md:h-14' : 'w-16 h-16 md:w-24 md:h-24')}
                        style={{
                          backgroundColor: `${colors.primary}14`,
                          border: `1px solid ${colors.primary}66`,
                          boxShadow: `0 0 ${isSub ? '18px' : '30px'} -8px ${colors.primary}80`,
                          color: colors.primary,
                        }}
                      >
                        <div className={cn(isSub ? 'absolute inset-1.5' : 'absolute inset-2')}>
                          <FactionLogo
                            faction={faction.id}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-baseline gap-2 md:gap-3 flex-wrap mb-1">
                        <h2 className={cn('font-russo font-black text-white', isSub ? 'text-base md:text-lg' : 'text-xl md:text-2xl')}>
                          {faction.name}
                        </h2>
                        {isSub && parentName && (
                          <span
                            className="font-ibm-mono text-[9px] md:text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded-sm border"
                            style={{ color: parentColor, borderColor: `${parentColor}55` }}
                          >
                            Подфракция «{parentName}»
                          </span>
                        )}
                      </div>
                      ...
                      {faction.motto && (
                        <p
                          className={cn('font-oswald italic flex items-center gap-2', isSub ? 'text-xs mb-1.5' : 'text-sm md:text-base mb-2.5')}
                          style={{ color: colors.primary }}
                        >
                          <Quote className={cn('shrink-0', isSub ? 'w-3 h-3' : 'w-3.5 h-3.5')} />
                          {faction.motto}
                        </p>
                      )}
                      ...
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        {faction.homeWorld && (
                          <span className="inline-flex items-center gap-1.5 font-ibm-mono text-[10px] md:text-[11px] px-2 py-0.5 rounded bg-military-charcoal/70 border border-military-steel/40 text-military-taupe uppercase tracking-wider">
