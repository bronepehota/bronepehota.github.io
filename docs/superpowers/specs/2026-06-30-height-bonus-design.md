...
- **Без заголовка-лейбла** (подтверждено по мокапу): таблетки сами по себе подписаны
  («с тыла»/«прицельный»/«с высоты»), отдельный «МОДИФИКАТОРЫ» сверху — лишний шум и лишняя строка
  по высоте. Контейнер pill'ов идёт напрямую.
...
- Pill'ы: `surprise` (shot+melee), `aimed` (shot, squad-only), `height` (shot, по гейту A2).
  (Для melee в баре только surprise; бар рендерится, когда есть хотя бы один применимый pill.)
...
- **Высота бара** минимальна: без лейбла это одна строка pill'ов (wraps при тесноте) + полная
  execute-кнопка ниже. По высоте ~нейтрально к текущему инлайн-ряду; `flex-wrap` не ломает layout при 3+ модификаторах.
